...
$manufacturerId = $_SESSION['user_id'];
$loggedInUserName = $_SESSION['username'] ?? 'Manufacturer';
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Manufacturer Dashboard - Pending Orders</title>
...
<body>
    ...
    <div class="container-fluid">
        <div class="header-section">
            <h1>Manufacturer Dashboard - Pending Orders</h1>
...
<script>
    ...
    const MANUFACTURER_ID = <?php echo json_encode($manufacturerId); ?>;
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if (customConfirmModalElement) {
            ...
            showAlert("Error", "Critical: Custom confirm modal structure missing. Please contact support.", "danger");
        }
        ...
        if (rejectReasonModalElement) {
            ...
            showAlert("Error", "Critical: Reject reason modal structure missing. Please contact support.", "danger");
        }
    ...
    async function loadPendingOrders() {
        ...
        try {
            ...
            const pendingOrders = await fetchData(`${API_BASE_URL_ORDER}/manufacturer/${MANUFACTURER_ID}?status=Pending`);
            ...
            showAlert('Error', `Failed to load orders: ${error.message}`, 'danger');
        }
    }
    ...
    function showCustomConfirm(orderId, newStatus, distributorName) {
        if (!customConfirmModalInstance) {
            ...
            showAlert("Error", "Confirmation modal not ready. Please refresh.", "danger");
            return;
        }
    ...
    function showRejectReasonModal(orderId, distributorName) {
        if (!rejectReasonModalInstance) {
            ...
            showAlert("Error", "Rejection modal not ready. Please refresh.", "danger");
            return;
        }
     ...
     async function handleOrderAction(orderId, newStatus, reason = '') {
            let title = newStatus === 'Rejected' ? 'Rejected' : 'Success';
            let successMessage = newStatus === 'Rejected' 
                ? `Order ${orderId} rejected successfully with reason: "${reason}"` 
                : `Order ${orderId} accepted successfully!`;
            let alertType = newStatus === 'Rejected' ? 'rejected' : 'success';
            ...
            try {
                await fetchData(`${API_BASE_URL_ORDER}/update-status/${orderId}`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload)
                });

                showAlert(title, successMessage, alertType);
                loadPendingOrders();

            } catch (error) {
                console.error(`Error during ${newStatus} action:`, error);
                showAlert('Error', `Failed to ${newStatus.toLowerCase()} order ${orderId}. ${error.message}`, 'danger');
            }
        }
</script>
